...
echo "Using private_key at: ${SSH_PRIVATE_KEY_PATH}"
...
while true; do
  ...
  if [ "$1" == "push" ] && [ "$2" == "directory" ]; then
    if ! ssh -i "$SSH_PRIVATE_KEY_PATH" "$CHID"@"$hostname" "[ -d $path_to_e3 ]"  > /dev/null 2>&1; then
    ...
    if ! ssh -i "$SSH_PRIVATE_KEY_PATH" "$CHID"@"$hostname" "[ -f $path_to_e3 ]"  > /dev/null 2>&1; then
...
if [ "$1" == "push" ]; then
  if rsync -a -e "ssh -i $SSH_PRIVATE_KEY_PATH" "$path_to_local" "$CHID@$hostname:$path_to_e3" > /dev/null 2>&1; then
  ...
  if rsync -a -e "ssh -i $SSH_PRIVATE_KEY_PATH" "$CHID@$hostname:$path_to_e3" "$path_to_local" > /dev/null 2>&1; then
